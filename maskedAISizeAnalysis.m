...
for i=1:length(mm)
	clear task taskB taskW md sM stimuli eL;
	load(mm{i},'task','taskB','taskW','md','sM'); fprintf('\n=>=> Loaded: %s\n', mm{i});
	figure(figH);
	...
	contrastOut			= [info.contrastOut];
	pedestal			= [info.pedestal];
...
if st.doModelComparison
	...
	figure(fh); subplot(1,3,1);histogram(realtask.response.contrastOut(TLRSim),40);hold on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function doPlotRaw()
		...
		idxBr = [info.contrastOut] == 1;
		idxD = [info.contrastOut] == 0;
		idxNO = task.response == NOSEE;
		idxYESB = task.response == YESBRIGHT;
		idxYESD = task.response == YESDARK;
