...
if useStaircase
	...
	PMB = PAL_AMUD_setupUD('up', 2, 'down', 1, 'xMin', 0,...
		'xMax', 0.5, 'stepSizeUp', 0.1, 'stepSizeDown', 0.05,...
		'startValue', 0, 'stopCriterion', stopCriterion, 'stopRule', stopRule);
	
	PMW = PAL_AMUD_setupUD('up', 1, 'down', 2, 'xMin', 0.5,...
		'xMax', 1, 'stepSizeUp', 0.05, 'stepSizeDown', 0.1,...
		'startValue', 1, 'stopCriterion', stopCriterion, 'stopRule', stopRule);
...
try
	...
	while breakloop ~= true
		...
		drawBackground(sM);
		Screen('DrawText',sM.win,'GREATER THAN (Left) or LESS THAN (Right)',0,0);
		...
		if response > -1
			if bG == 0.3
				if ~PMB.stop
					PMB = PAL_AMUD_updateUD(PMB,response);
					...
					t = sprintf('C=%.2g | R=%i | T=%i | BG=%.2g',PMB.x(end),response,loop,bG);
					Screen('DrawText',sM.win,t,0,0);
				end
				Screen('Flip',sM.win); %flip the buffer
			else
				if ~PMW.stop
					PMW = PAL_AMUD_updateUD(PMW,response);
					...
					t = sprintf('C=%.2g | R=%i | T=%i | BG=%.2g',PMW.x(end),response,loop,bG);
					Screen('DrawText',sM.win,t,0,0);
				end
				Screen('Flip',sM.win); %flip the buffer
			end
