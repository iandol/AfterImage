...
subject = ans{1};
lab = 'labAOC_aristotle'; %dorris lab or our machine?
comments = ans{2};
useStaircase = false;
stimTime = 6;
pedestalTime = 0.5;
...
%% add by Xu,2018 0508
pedestalRange = [2.5:0.1:3.5]; % sizerange, Xu 2018
repeatTime = 2;
pedestalColorB = 0.3;   % manually enter
pedestalColorW = 0.8;
...
if strcmpi(lab,'labAOC_aristotle')
    ...
    % 	pedestalBlackLinear = discSize - fliplr(pedestalRange);
    % 	pedestalWhiteLinear = discSize + pedestalRange;
    % 	pedestalBlack =  pedestalBlackLinear;
    % 	pedestalWhite = pedestalWhiteLinear; % changed by Xu,20180508
elseif strcmpi(lab,'dorrislab_aristotle')
    ...
    % 	pedestalWhite = [ 0.5    0.5490    0.5961    0.6431    0.6902    0.7373    0.7843    0.8314]; % changed by Xu,20180508
else
    calibrationFile='';
    if isstruct(calibrationFile) %older matlab version bug wraps object in a structure
        calibrationFile = calibrationFile.c;
    ...
    % 	pedestalWhite = [0.6863    0.7216    0.7490    0.7765    0.8000    0.8275    0.8510    0.8745]; % changed by Xu,20180508
end
...
task.name = nameExp;
task.nBlocks = repeatTime*length(pedestalRange); % changed by Xu,20180508
...
taskW.name = nameExp;
taskW.nBlocks = repeatTime; % changed by Xu,20180508
...
taskB.name = nameExp;
taskB.nBlocks = repeatTime; % changed by Xu,20180508
...
try %our main experimental try catch loop
    ...
    box on; grid on; grid minor; ylim([2.5 3.5]);
    ...
    while ~breakloop && task.totalRuns <= task.nRuns
        ...
        stimuli{1}.size = discSize;
        stimuli{1}.sigma = 3;
        hide(stimuli);
        response = NaN;
        stimuli.showMask = false;
        colourOut = task.outValues{task.totalRuns,1};
        stimuli{1}.colour = colourOut;
        if colourOut == 0
            pedestalColor = pedestalColorB; % add by Xu,20180508
            pedestal = taskB.outValues{taskB.totalRuns,1};
            pedestalLinear = pedestalBlackLinear(pedestalBlack==pedestal);
        else
            pedestalColor = pedestalColorW; % add by Xu,20180508
        ...
        while strcmpi(fixated,'fix') %initial fixation held
            ...
            %% add by Xu, 2018.05.08
            stimuli{1}.colour = pedestalColor;
            stimuli{1}.size = pedestal;
            stimuli{1}.sigma = 3;
            setup(stimuli,s); %setup our stimulus object
            stimuli{1}.xPositionOut = XPos(posloop);
            stimuli{1}.yPositionOut = YPos(posloop);
    ...
    function doPlot()
        ...
        cla; line([min(x) max(x)],[3 3],'LineStyle','--','LineWidth',1);	hold on
