...
sigma = 3;
discSize = 3;
pedestalColorB = 0.5-0.28;   % manually enter
pedestalColorW = 0.5+0.28;
if useStaircase == true
	PF = @PAL_Gumbel;
	pedestalRange = 0.8:0.02:1.2;
else
	pedestalRange = 0.8:0.04:1.2;
end
nBlocks = 5;
...
if strcmpi(lab,'lab305_aristotle')
	...
	useEyeLink = true;
	isDummy = false;
	pixelsPerCm = 27; %27 is Display++; 26 for D-lab,32=Lab CRT -- 44=27"monitor or Macbook Pro
...
%=====================================================================
	...
	while ~breakloop && task.thisRun <= task.nRuns
		...
		if useStaircase == true
			if colourOut == 0
				stimuli{1}.colourOut = 0;
                pedestalColor = pedestalColorB; % add by Xu,20180508
				pedestal = 3*staircaseB.xCurrent; % size if pedistal
			else
				stimuli{1}.colourOut = 1;
                pedestalColor = pedestalColorW; % add by Xu,20180508
				pedestal = 3*staircaseW.xCurrent;
			end
		else
			if colourOut == 0
				stimuli{1}.colourOut = 0;
                pedestalColor = pedestalColorB; % add by Xu,20180508
				pedestal = 3*taskB.outValues{taskB.thisRun,1};
			else
				stimuli{1}.colourOut = 1;
                pedestalColor = pedestalColorW; % add by Xu,20180508
				pedestal = 3*taskW.outValues{taskW.thisRun,1};
			end
		end
		...
		while strcmpi(fixated,'fix') %initial fixation held
            ...
            stimuli{1}.colourOut = pedestalColor;
			tPedestal=GetSecs;
